...
// -- kalau diubah, ingat sinkronkan ke sana & AuditPoOverseasPage.tsx juga.
...
// AuditPoOverseasPage.tsx juga) supaya ada kesan highlight/glossy dari tengah ke tepi.
...
function DashboardModal({ onClose }: { onClose: () => void }) {
  ...
  // kalau diubah, ingat sinkronkan ke sana & AuditPoPage.tsx juga) -- chart batang HORIZONTAL
  ...
  useEffect(() => {
    fetchDistinctNamaPt('audit_po_pi_local_comp').then(setPtOptions);
  }, []);
  ...
  // "GENERAL" dikeluarkan dari chart ini (bukan nama PT spesifik) -- semua `PT_OPTIONS` lain
  // selalu jadi baris dgn count default 0 (chart tetap tampil apa adanya walau rentang kosong).
  const fetchVendorStats = useCallback(async (from: string, to: string) => {
    setVendorLoading(true);
    setVendorError(null);
    const { data, error: fetchError } = await supabase.from('audit_po_pi_local_comp').select('nama_pt')
      .not('status_audit', 'is', null)
      .gte('created_at', `${from}T00:00:00`).lte('created_at', `${to}T23:59:59`);
  ...
  // `KATEGORI_OPTIONS` -- HANYA kategori yg BENERAN ada datanya yg ditampilkan (descending).
  const fetchKategoriStats = useCallback(async (from: string, to: string) => {
    setKategoriLoading(true);
    setKategoriError(null);
    const { data, error: fetchError } = await supabase.from('audit_po_pi_local_comp').select('kategori')
                  ...
                  <svg width={600} height={300} viewBox="0 0 600 300">
                    {(() => {
                      ...
                      return (
                        <>
                          <defs>
                            <filter id="piLocalPieShadow" x="-30%" y="-30%" width="160%" height="160%">
                              <feDropShadow dx="0" dy="6" stdDeviation="6" floodColor="#000000" floodOpacity="0.25" />
                            </filter>
                            {slices.map(s => (
                              <radialGradient key={`grad-${s.label}`} id={`piLocalPieGrad-${s.label.replace(/\s+/g, '')}`} cx="35%" cy="30%" r="75%">
                          ...
                          <g filter="url(#piLocalPieShadow)">
                            {slices.map(s => {
                              if (s.pct <= 0.05) return null;
                              const gradId = `url(#piLocalPieGrad-${s.label.replace(/\s+/g, '')})`;
...
// diubah, ingat sinkronkan ke sana & AuditPoOverseasPage.tsx juga.
...
// diubah, ingat sinkronkan ke sana & AuditPoPage.tsx juga) -- chart batang HORIZONTAL jumlah
      ...
      <main className="px-3 pt-2 pb-2 flex-1 flex flex-col overflow-hidden">
        <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm overflow-hidden flex-1 flex flex-col min-h-0">
          ...
          <div className="overflow-x-auto overflow-y-auto flex-1 min-h-0">
            <table className="w-full text-[11px] bg-white table-fixed min-w-[1150px]">
              <colgroup>
