...
import { Clock, ArrowUp, Tag, ChevronDown, ChevronUp, ArrowLeft } from 'lucide-react';
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-bold text-2xl text-slate-900 leading-tight">Tarif Kontrak Vendor</h1>
              <p className="text-slate-700 text-sm mt-1">Kelola master tarif dari vendor (Sea & Air).</p>
            </div>
          </div>
          <button 
            onClick={() => openModal()}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex-1 md:w-64">
              <label className="block text-xs font-semibold text-slate-500 mb-1">Cari Vendor / Jasa</label>
              <input 
                type="text"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Vendor</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Kategori</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Jasa</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Shipment Type</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Satuan</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Tarif Type</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Nominal</th>
                  <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">PPN Status</th>
                ...
                {loading ? (
                  ...
                  filteredData.map((rec) => (
                    <tr key={rec.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-3 text-sm text-slate-800 font-semibold">{rec.vendor_name}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-700 font-medium">
                        ...
                        {(rec.kali_hari_juga || rec.round_up_qty || rec.double_charge_threshold || rec.alias_names || rec.threshold_basis_qty) && (
                          <div className="flex flex-wrap items-center gap-1 mt-1">
                            {rec.kali_hari_juga && <span className="inline-flex items-center text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded border border-slate-200" title="Dikali Jumlah Hari"><Clock size={10} className="mr-0.5"/> x Hari</span>}
                            {rec.round_up_qty && <span className="inline-flex items-center text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded border border-slate-200" title="Pembulatan ke atas"><ArrowUp size={10} className="mr-0.5"/> Round Up</span>}
...
{rec.alias_names && <span className="inline-flex items-center text-[10px] bg-slate-100 text-slate-600 px-1 py-0.5 rounded border border-slate-200" title={`Alias: ${rec.alias_names}`}><Tag size={10} className="mr-0.5"/> Alias</span>}
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600">{rec.shipment_type || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 font-mono">{rec.satuan || '-'}</td>
                      <td className="px-4 py-3 text-xs text-slate-600">{rec.tarif_type}</td>
                      <td className="px-4 py-3 text-sm font-mono text-slate-800 text-right font-medium whitespace-nowrap">
                      ...
                      <td className="px-4 py-3 text-[10px] text-slate-500 text-center uppercase">{rec.ppn_status.replace('_', ' ')}</td>
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button 
                            onClick={() => openModal(rec)}
                            className="text-xs bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 hover:text-blue-600 font-medium px-2 py-1 rounded transition-colors shadow-sm"
                          >
                            Edit
                          </button>
                          <button 
                            onClick={() => handleNonaktifkan(rec.id)}
                            className="text-xs bg-white border border-slate-200 hover:bg-red-50 text-slate-600 hover:text-red-600 font-medium px-2 py-1 rounded transition-colors shadow-sm"
                          >
                            Nonaktifkan
                          </button>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto bg-navy-900/70 backdrop-blur-sm pt-10">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl my-6 animate-fade-up">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div>
                <h3 className="font-bold text-slate-900 text-lg">{editingId ? 'Edit Tarif Kontrak' : 'Tambah Tarif Baru'}</h3>
              </div>
              <button
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                    <h4 className="text-sm font-bold text-slate-700 border-b border-slate-200 pb-2">Informasi Jasa</h4>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Nama Jasa <span className="text-red-500">*</span></label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Shipment Type</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Container Line</label>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                    <h4 className="text-sm font-bold text-slate-700 border-b border-slate-200 pb-2">Kontrak & PPN</h4>
                    
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Status PPN <span className="text-red-500">*</span></label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">PPN Pct (%)</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">No Quotation</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Tgl Terbit</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Tgl Berlaku</label>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                    <h4 className="text-sm font-bold text-slate-700 border-b border-slate-200 pb-2">Detail Tarif</h4>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Tarif Type <span className="text-red-500">*</span></label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Mata Uang <span className="text-red-500">*</span></label>
                    ...
                    {!isNominalHidden && (
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Nominal Tarif <span className="text-red-500">*</span></label>
                    ...
                    {tarifType === 'PERCENTAGE' && (
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Persentase (Pct Rate) <span className="text-red-500">*</span></label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Range Min (opsional)</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Range Max (opsional)</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Min Charge (opsional)</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-1">Min Charge Satuan</label>
                  ...
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
                    <h4 className="text-sm font-bold text-slate-700 border-b border-slate-200 pb-2">Tambahan</h4>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-1">Catatan Internal</label>
              ...
              <div className="mt-6 bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
                <button 
                  ...
                  className="w-full flex items-center justify-between p-4 bg-slate-100 hover:bg-slate-200 transition-colors text-slate-700 font-bold text-sm"
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       ...
                       <div className="space-y-5">
                         <div>
                            <label className="flex items-start gap-3 cursor-pointer group">
                               ...
                               <div>
                                 <span className="text-sm font-semibold text-slate-800 block group-hover:text-blue-600 transition-colors">Kalikan dengan jumlah hari juga</span>
                                 <span className="text-[11px] text-slate-500 leading-tight block mt-1">Aktifkan kalau tarif ini dihitung 2 dimensi, cth CBM DAN jumlah hari sekaligus (tarif x max(CBM,minimum) x jumlah hari). Kalau satuan sudah '/Hari' saja, TIDAK perlu diaktifkan.</span>
                               </div>
                            </label>
                         </div>
                         <div>
                            <label className="flex items-start gap-3 cursor-pointer group">
                               ...
                               <div>
                                 <span className="text-sm font-semibold text-slate-800 block group-hover:text-blue-600 transition-colors">Bulatkan ke atas (round up)</span>
                                 <span className="text-[11px] text-slate-500 leading-tight block mt-1">Aktifkan kalau nilai CBM/Revton dibulatkan ke atas ke bilangan bulat sebelum dikalikan tarif (cth 4.23 CBM jadi 5 CBM).</span>
                               </div>
                            </label>
                         </div>
                       </div>
                       ...
                       <div className="space-y-5">
                          <div className="bg-white p-3 rounded-lg border border-slate-200">
                             <h5 className="text-sm font-bold text-slate-700 mb-3">Biaya Ganda (Double Charge)</h5>
                             <div className="flex items-center gap-3 mb-2">
                               <div className="flex-1">
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">Ambang CBM</label>
                               ...
                               <div className="w-24">
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">Pengali</label>
                                  <input type="number" step="any" disabled={!doubleChargeThreshold} value={doubleChargeMultiplier} onChange={e => setDoubleChargeMultiplier(e.target.value)} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 disabled:bg-slate-100 disabled:text-slate-400" />
                               </div>
                             </div>
                             <p className="text-[11px] text-slate-500 leading-tight">Kalau CBM shipment melebihi ambang ini, nominal tarif dikalikan sejumlah Pengali (cth ambang=6, pengali=2 artinya CBM&gt;6 maka tarif dikali 2).</p>
                          </div>
                          
                          <div className="bg-white p-3 rounded-lg border border-slate-200">
                             <h5 className="text-sm font-bold text-slate-700 mb-2">Ambang Kuantitas Dasar</h5>
                             <div className="mb-2">
                             ...
                             <p className="text-[11px] text-slate-500 leading-tight">Kuantitas yang SUDAH TERMASUK tarif dasar sebelum biaya tambahan berlaku (cth 10 untuk 'Max 10 Item', 5 untuk '0 s/d 5 CBM', 250 untuk '0 s/d 250 Kgs'). Baris tarif lain dengan nama mengandung 'Additional' akan otomatis dipakai untuk kelebihan di atas ambang ini.</p>
                          </div>
                          
                          <div>
                            <label className="block text-sm font-semibold text-slate-800 mb-1">Nama Alternatif</label>
                            ...
                            <p className="text-[11px] text-slate-500 leading-tight mt-1">Nama lain yang mungkin dipakai di invoice vendor untuk baris tarif yang SAMA (istilah beda tapi maksudnya sama). Sistem akan mencocokkan baris invoice ke SALAH SATU nama ini juga, bukan cuma Nama Jasa.</p>
                          </div>
                       </div>
                    </div>
                  </div>
