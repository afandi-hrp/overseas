...
function humanizeUploadError(raw: string): string {
  ...
  if (msg.includes('timeout') || msg.includes('timed out') || msg.includes('aborted')) {
    return 'The automation server did not respond in a reasonable time (timeout). The document may still be processing in the background — check back shortly.';
  }
...
// Dipakai dari tombol "Upload Additional Doc" di dalam Document Completeness Checklist (Audit
// Courier, SharedDataTable.tsx) -- utk kirim ulang dokumen susulan yang belum terupload saat
// upload pertama di halaman Upload Courier. Payload-nya dikirim ke webhook n8n yang SAMA dgn
// UploadPage.tsx (X-Webhook-Type: 'courier'), TAPI ditambah field `awb_hint` (REPLIKA PERSIS pola
// `no_po_hint` milik BunkerUploadModal.tsx) supaya dokumen susulan (yang belum tentu mencantumkan
// AWB di dalamnya sendiri, mis. Credit Note) tetap bisa digabung ke record PIB/CN yang benar,
// bukan dianggap shipment baru. server.ts WAJIB forward field ini (lihat komentar di sana).
export default function CourierUploadSusulanModal({ onClose, onJobStarted, onSentNoJob, awbHint }: {
  onClose: () => void;
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[80] flex justify-center items-center p-2 sm:p-4">
      <div className="bg-white w-[70vw] max-w-3xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center p-4 sm:px-6 sm:py-4 border-b border-slate-200 shrink-0">
          <div>
            <h2 className="text-base font-bold tracking-tight text-[#5A305A]">Upload Additional Document — Courier</h2>
            <p className="text-xs font-light text-[#5A305A] mt-0.5">For supplementary documents missed during the first upload (e.g. Credit Note, SPTNP, BPN SPTNP)</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {awbHint ? (
            <div className="mb-4 p-3 rounded-xl bg-blue-50 border border-blue-200 text-xs text-blue-800">
              This document will automatically be merged into <span className="font-bold">AWB: {awbHint}</span>, even if the document itself doesn't mention the AWB (e.g. a Credit Note).
            </div>
          ) : (
            <div className="mb-4 p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-800">
              This record doesn't have an AWB number yet, so the document will be processed like a normal first-time upload instead of being merged automatically.
            </div>
          )}
          ...
          >
            ...
            <p className="text-xs text-[#5A305A] mt-1">
              You can upload just the missing document type(s), no need to resend everything.
            </p>
          </div>
