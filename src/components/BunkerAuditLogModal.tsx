...
//
// Filter `splitAuditCatatan(e.catatan)` SAJA (2026-09, GANTI dari `... || e.user_email`) --
// versi lama meloloskan entri kalau SALAH SATU syarat terpenuhi (parse sukses ATAU user_email
// terisi), TERNYATA masih tembus entri "asing" dari trigger DB (`fn_audit_bunker_dokumen`,
// lihat CLAUDE.md "Bunker & Courier/Sea & Air -- Audit Trail") — trigger itu jalan utk SEMUA
// UPDATE termasuk yang dipicu user login (bukan cuma n8n), jadi `user_email`-nya JUGA terisi
// (SAMA persis email user yg lagi login), catatan-nya tetap dump mentah `fn_audit_diff`
// (mis. `status_manual: {} -> {...}`) yang gagal diparse `splitAuditCatatan`. Kondisi `||` lama
// meloloskannya krn `user_email` terisi, walau `catatan`-nya bukan format aplikasi. Fix: HANYA
// andalkan hasil parse `splitAuditCatatan` (SAMA prinsip `TRAIL_APP_WRITTEN_FILTER` di
// `SharedDataTable.tsx` utk halaman Audit Trail global) -- `logBunkerAudit()` SATU-SATUNYA
// fungsi yang menulis ke tabel ini dari app, SELALU format ketat "{field} — Lama: X → Baru: Y",
// jadi entri manapun yang GAGAL diparse itu (terlepas `user_email`-nya terisi atau tidak) BUKAN
// dari app ini, disembunyikan (bukan dihapus dari DB, murni tidak dirender).
