...
function humanizeUploadError(raw: string): string {
  ...
  if (msg.includes('timeout') || msg.includes('timed out') || msg.includes('aborted')) {
    return 'The automation server did not respond in time (timeout). The document may still be processing in the background — check the processing queue again in a moment.';
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.job_id) {
        ...
        onSentNoJob(data.pesan || 'Document sent, but the status could not be confirmed yet.', true);
      } else {
        onSentNoJob('Document successfully sent to the processing queue.', false);
      }
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[70] flex justify-center items-center p-2 sm:p-4">
      <div className="bg-white w-[70vw] max-w-3xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-center p-4 sm:px-6 sm:py-4 border-b border-slate-200 shrink-0">
          <div>
            <h2 className="text-base font-bold tracking-tight text-[#5A305A]">Upload Document — FAR Overseas Air</h2>
            <p className="text-xs font-light text-[#5A305A] mt-0.5">Freight invoice, PO (multiple allowed), supporting photos</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          >
            ...
            <p className="font-semibold text-sm text-[#5A305A]">
              {dragging ? 'Drop files here...' : files.length === 0 ? 'Click or drag & drop files (PDF, JPG, PNG)' : 'Click to add more files'}
            </p>
            <p className="text-xs text-[#5A305A] mt-1">
              All files can be combined or sent separately, AI will sort them automatically.
            </p>
          </div>
          ...
          <p className="text-[10px] font-bold text-[#5A305A] uppercase tracking-widest mt-5 mb-3">Step 2 — Process & Send</p>
          <button
